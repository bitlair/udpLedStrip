...
void EtherCard::sendUdp (char *data, word datalen, word sport, byte *dip
  , word dport) 
{
  udpPrepare(sport, dip, dport);
  if (datalen > (word) (ether.bufferSize - UDP_DATA_P))
  {
    datalen = ether.bufferSize - UDP_DATA_P;
  }
  memcpy(gPB + UDP_DATA_P, data, datalen);
  udpTransmit(datalen);
}

void EtherCard::sendBufferUdp(word datalen, word sport, byte *dip, word dport) 
{
  udpPrepare(sport, dip, dport);
  udpTransmit(datalen);
}
...
void EtherCard::browseUrl (prog_char *urlbuf, const char *urlbuf_varpart, prog_char *hoststr, void (*callback)(byte,word,word)) {
  browseUrl(urlbuf, urlbuf_varpart, hoststr, (prog_char*) PSTR("Accept: text/html"), callback);
}
